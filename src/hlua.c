...
__LJMP static int hlua_run_sample_fetch(lua_State *L)
{
	...
	if (!f->process(s->p, s->s->sess, s->s, 0, args, &smp, f->kw, f->private)) {
		if (s->stringsafe)
			lua_pushstring(L, "");
		else
			lua_pushnil(L);
...
static int hlua_http_req_set_meth(lua_State *L)
{
	...
	lua_pushboolean(L, http_replace_req_line(0, name, name_len, s->p, s->s) != -1);
	return 1;
}
...
static int hlua_http_req_set_path(lua_State *L)
{
	...
	lua_pushboolean(L, http_replace_req_line(1, name, name_len, s->p, s->s) != -1);
	return 1;
}
...
static int hlua_http_req_set_query(lua_State *L)
{
	...
	lua_pushboolean(L, http_replace_req_line(2, trash.str, trash.len, s->p, s->s) != -1);
	return 1;
}
...
static int hlua_http_req_set_uri(lua_State *L)
{
	...
	lua_pushboolean(L, http_replace_req_line(3, name, name_len, s->p, s->s) != -1);
	return 1;
}
